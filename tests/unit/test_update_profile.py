...
def basic_info_data():
    return {'email':'petar@gmail.com', 'first_name':'Petar', 'last_name':'Peric', 'phone_number':'654321', 'birthday':'2001-04-30'}

@pytest.fixture(scope='function')
def work_experience_data():
    return {"title": "Strucna prak", "type": "INTERNSHIP", "company": "devops & co", "location": "Izmisljenog junaka 8", "currently_working": False, "start_date": "2015-05-15", "end_date": "2018-05-15"}

@pytest.fixture(scope='function')
def education_data():
    return {"education": "Master's Degree", "school":  "FTN", "degree": "Master", "field_of_study": "Software Engineering", "start_date": "2014-05-15" }
...
class TestEditProfile:
    ...
    def test_edit_basic_info_success(self, app: Flask, public_profile: Profile, basic_info_data:dict):
        # with app.app_context(): # no need to use context, context is already set
        updated = profile_service.edit_basic_info(data=basic_info_data, profile=public_profile)
        ...
        assert basic_info_data['birthday'].strftime('%Y-%m-%d') == updated.birthday.strftime('%Y-%m-%d')




    def test_edit_work_experience_success(self, app: Flask, public_profile: Profile, work_experience_data:dict):
        updated_exp = profile_service.create_or_update_work_experience(data=work_experience_data, profile=public_profile)
        
        assert public_profile.id == updated_exp.profile_id
        assert work_experience_data['title'] == updated_exp.title
        assert work_experience_data['type'] == updated_exp.type.name # name is to get enum str representation
        assert work_experience_data['title'] == updated_exp.title
        assert work_experience_data['company'] == updated_exp.company
        assert work_experience_data['location'] == updated_exp.location
        assert work_experience_data['currently_working'] == updated_exp.currently_working
        assert work_experience_data['start_date'].strftime('%Y-%m-%d') == updated_exp.start_date.strftime('%Y-%m-%d')
        assert work_experience_data['end_date'].strftime('%Y-%m-%d') == updated_exp.end_date.strftime('%Y-%m-%d')


    def test_edit_education_success(self, app: Flask, public_profile: Profile, education_data:dict):
        updated_edu = profile_service.create_or_update_education(data=education_data, profile=public_profile)
        ...
        assert education_data['start_date'] == updated_edu.start_date.strftime('%Y-%m-%d')
...
class TestEditProfileKeys:
    ...
    Sending bad keys should raise keyerror exception.
    Sending non existing keys shouldn't make a difference. Request should fail
    '''

    def test_edit_basic_info_bad_keys(self, app:Flask, public_profile: Profile, trash_data:dict):
        with pytest.raises(KeyError):
            profile_service.edit_basic_info(data=trash_data, profile=public_profile)


    def test_edit_basic_info_extra_keys(self, app:Flask, public_profile: Profile, basic_info_data:dict, trash_data:dict):
        basic_info_data_with_trash_data = {**basic_info_data, **trash_data}
        updated = profile_service.edit_basic_info(data= basic_info_data_with_trash_data, profile=public_profile)
        ...
        assert basic_info_data_with_trash_data['birthday'].strftime('%Y-%m-%d') == updated.birthday.strftime('%Y-%m-%d')


    def test_edit_work_experience_bad_keys(self, app:Flask, public_profile: Profile, trash_data:dict):
        with pytest.raises(KeyError):
            profile_service.create_or_update_work_experience(data=trash_data, profile=public_profile)


    def test_edit_work_experience_extra_keys(self, app:Flask, public_profile: Profile, work_experience_data:dict, trash_data:dict):
        work_experience_data_with_trash_data = {**work_experience_data, **trash_data}
        updated_exp = profile_service.create_or_update_work_experience(data= work_experience_data_with_trash_data, profile=public_profile)
    ...
    def test_edit_education_extra_keys(self, app:Flask, public_profile: Profile, education_data:dict, trash_data:dict):
        education_data_with_trash_data = {**education_data, **trash_data}
        updated_edu = profile_service.create_or_update_education(data= education_data_with_trash_data, profile=public_profile)
