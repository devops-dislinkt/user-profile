...
def seed_db():
    ...
    kiki = Profile({ 'username':"kiki_test", 'email':'kiki@gmail.com', 'first_name':'Kiki', 'last_name':'Mikic', 'phone_number':'123456', 'birthday':'1920-04-30','biography':'Kikina biografija','private':True})
...
class TestBlockProfile:
    ...
    def test_block_one_profile_success(self, app:Flask, mika: Profile, kiki: Profile):
        ...
        mika ->X kiki
        '''
        block = profile_service.block_profile(username_to_block=kiki.username, profile=mika)
    ...
    def test_block_multiple_profiles_success(self, app:Flask, pera: Profile, mika: Profile, zika: Profile, kiki: Profile):
        '''
        ...
        zika ->X kiki
             ->X mika
        '''
        pera_blocks_zika = profile_service.block_profile(username_to_block=zika.username, profile=pera)
        pera_blocks_kiki = profile_service.block_profile(username_to_block=kiki.username, profile=pera)
        zika_blocks_kiki = profile_service.block_profile(username_to_block=kiki.username, profile=zika)
        zika_blocks_mika = profile_service.block_profile(username_to_block=mika.username, profile=zika)
