...
class TestViewProfile:
    ...
    def test_view_public_profile_loggedin(self, client: FlaskClient):
        response = client.get(
            f"/api/profile/{PUBLIC_VALID_ID}?username={PRIVATE_PROFILE_USER_2}"
        )
    ...
    def test_view_private_profile_loggedin(self, client: FlaskClient):
        response = client.get(
            f"/api/profile/{PRIVATE_VALID_ID}?username={PUBLIC_PROFILE_USER_2}"
        )
    ...
    def test_view_private_profile_follower(self, client: FlaskClient):
        response = client.get(
            f"/api/profile/{PRIVATE_VALID_ID}?username={PRIVATE_PROFILE_USER_1}"
        )
    ...
    def test_view_private_profile_follower_approved(self, client: FlaskClient):
        response = client.get(
            f"/api/profile/{PRIVATE_VALID_ID}?username={PUBLIC_PROFILE_USER_1}"
        )
        profiles = Profile.query.all()
        print(profiles[1].id)
        assert response.status_code == 200
