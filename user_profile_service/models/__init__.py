...
class Profile(db.Model, SerializerMixin):
    ...
    followers: InstrumentedList = db.relationship(
        "Following",
        backref=db.backref("following", uselist=True),
        primaryjoin=id == Following.following_id,
        uselist=True,
    )
    following: InstrumentedList = db.relationship(
        "Following",
        backref=db.backref("follower", uselist=True),
        primaryjoin=id == Following.follower_id,
        uselist=True,
    )

    #  a list of the profiles that you've blocked
    profiles_blocked_by_me: InstrumentedList = db.relationship(
        "Blocking", primaryjoin=id == Blocking.blocker_id, uselist=True
    )
